...
let swiftui_source_path = "../swiftui/OCamlDemoSwiftUI.swift"
let swiftui_mac_source_path = "../swiftui/OCamlDemoSwiftUIMac.swift"
let swiftui_backend_source_path = "../swiftui/ocaml_demo_apple_swiftui.ml"
...
let test_compact_sidebar_uses_chatgpt_drawer () =
  ...
  require
    (contains
       source
       ~substring:".ignoresSafeArea(.container, edges: .all)")
  ...
  require
    (not
       (contains
          source
          ~substring:
            "selectedRouteDetail(\n          suppressNativeToolbar: isCompactSidebarOpen || \
             isCompactSidebarDragging\n        )"))
    "compact sidebar drawer state should not be passed into the main route because it \
     invalidates the native List during open/close transitions";
  require
...
let test_compact_sidebar_state_lives_in_dedicated_view () =
  let source = read_file swiftui_source_path in
  require
    (contains source ~substring:"private struct OCamlDemoCompactSidebarSplitView: View")
    "compact sidebar transition state should live in a dedicated SwiftUI view so drawer \
     animation updates do not re-evaluate the giant OCamlDemoNodeView generic body";
  let node_view_start =
    match substring_index source ~substring:"private struct OCamlDemoNodeView: View" ~from:0 with
    | Some index -> index
    | None -> failwith "OCamlDemoNodeView not found"
  in
  let body_start =
    match substring_index source ~substring:"  var body: some View" ~from:node_view_start with
    | Some index -> index
    | None -> failwith "OCamlDemoNodeView body not found"
  in
  let node_view_state_source =
    String.sub source node_view_start (body_start - node_view_start)
  in
  require
    (not (contains node_view_state_source ~substring:"isCompactSidebarOpen"))
    "OCamlDemoNodeView should not own compact drawer open state; Time Profiler showed \
     sidebar toggles spending main-thread time in Swift protocol-conformance checks over \
     the full node view type"
;;

let test_compact_sidebar_avoids_type_erased_selected_route_boundary () =
  let source = read_file swiftui_source_path in
  require
    (not
       (contains
          source
          ~substring:"private func selectedRouteDetail(suppressNativeToolbar: Bool) -> AnyView"))
    "compact sidebar route detail should not use AnyView; profiling did not prove this \
     boundary helps, and type erasure hides SwiftUI structure"
;;

let test_compact_sidebar_composites_main_page_before_clipping () =
  let source = read_file swiftui_source_path in
  require
    (contains source ~substring:".compositingGroup()\n            .clipShape(")
    "compact sidebar should composite the moving main page before clipping/shadowing it; \
     simulator frame logs show transition hitches even when no lazy row bodies render"
;;

let test_compact_sidebar_has_env_gated_body_probe () =
  let source = read_file swiftui_source_path in
  require
    (contains source ~substring:"OCAML_DEMO_BODY_DEBUG")
    "body debug logging should be env-gated so production behavior is unchanged";
  require
    (contains source ~substring:"private final class OCamlDemoBodyProbe")
    "body debug logging should be centralized instead of ad hoc print statements";
  require
    (contains source ~substring:"mark(\"compact_split_body\")")
    "compact sidebar split body should be counted during transition profiling";
  require
    (contains source ~substring:"mark(\"compact_main_page\")")
    "compact sidebar main-page subtree should be counted during transition profiling";
  require
    (contains source ~substring:"mark(\"node_body\")")
    "OCamlDemoNodeView body should be counted during transition profiling";
  require
    (contains source ~substring:"mark(\"lazy_row_body\")")
    "lazy row bodies should share the same body-probe timeline as sidebar bodies"
;;
...
let test_compact_sidebar_uses_drawer_for_content_swipes () =
  let source = read_file swiftui_source_path in
  require
    (not (contains source ~substring:"ocaml_demoContentHorizontalSwipeActiveChanged"))
...
let test_compact_sidebar_does_not_install_closed_root_drag () =
  let source = read_file swiftui_source_path in
  require
    (contains source ~substring:"OCamlDemoCompactSidebarDrawerGestureModifier")
    "compact sidebar should use a dedicated modifier to install the drawer drag only when \
     it can handle an already-open drawer";
  require
    (contains source ~substring:"if isEnabled {\n      content.highPriorityGesture")
    "the drawer drag should be absent from the view tree while closed so journal scroll \
     keeps native List ownership";
  require
    (not (contains source ~substring:".highPriorityGesture(\n          DragGesture"))
    "compact sidebar should not attach an unconditional root DragGesture because it \
     competes with vertical scrolling"
;;

let test_compact_sidebar_uses_fast_logged_animation () =
  let source = read_file swiftui_source_path in
  require
    (contains source ~substring:".interactiveSpring(response: 0.16")
    "compact sidebar open/close should use a short response so it opens quickly";
  require
    (contains source ~substring:"\"compact_sidebar_set_open\"")
    "compact sidebar open/close should log state transitions for simulator debugging";
  require
    (contains source ~substring:"\"compact_sidebar_drag_start\"")
    "compact sidebar drag should log when it starts so gesture contention is debuggable"
;;

let test_compact_sidebar_has_native_stress_probe () =
  let source = read_file swiftui_source_path in
  require
    (contains source ~substring:"OCAML_DEMO_SIDEBAR_STRESS")
    "compact sidebar transition should have an env-gated native stress probe so simulator \
     perf logs do not depend on fragile host mouse coordinates";
  require
    (contains source ~substring:"runCompactSidebarStressIfNeeded")
    "compact sidebar stress should be centralized instead of adding ad hoc timers in the \
     view body";
  require
    (contains source ~substring:"setCompactSidebarOpen(nextOpen)")
    "compact sidebar stress should exercise the same logged open/close path as user \
     interaction"
;;
...
let test_horizontal_swipe_ignores_disabled_directions () =
  ...
  require
    (contains source
       ~substring:"guard node.horizontalSwipeRightEventId != nil else { return .zero }")
    "right swipe should not move row content when the right action is disabled";
  require
    (contains source
       ~substring:"guard node.horizontalSwipeLeftEventId != nil else { return .zero }")
...
let test_horizontal_swipe_ignores_vertical_scroll_before_state_update () =
  let source = read_file swiftui_source_path in
  require
    (contains source ~substring:"private func horizontalSwipeActiveTranslation")
    "horizontal swipe should centralize active-translation detection before updating \
     GestureState";
  require
    (contains source ~substring:"state = horizontalSwipeActiveTranslation(for: value.translation)")
    "vertical scroll gestures should not write their full translation into every row's \
     horizontal swipe GestureState";
  require
    (contains source ~substring:"guard horizontalSwipeActiveTranslation(for: value.translation) != .zero")
    "horizontal swipe should only haptic/send when the final translation resolves to an \
     active horizontal action"
;;
...
let test_native_list_uses_stable_node_identity () =
  ...
  require
    ...
    "SwiftUI List rows should use the stable OCamlDemoNode id, not the row offset, \
  ...
  require
    ...
    "focused-row scrolling should target the focused OCamlDemoNode id"
;;
...
let test_list_virtualization_probe_does_not_log_per_row_events () =
  ...
  require
    (contains source ~substring:"OCAML_DEMO_LIST_DEBUG")
...
let test_swiftui_library_builds_in_default_ios_context () =
  ...
  require
    ...
    "dune -x ios uses the default.ios target context; ocaml_demo_apple.swiftui must be \
     enabled there so iOS sysroot installs do not keep stale SwiftUI artifacts";
  require
    (contains source ~substring:"public_name ocaml_demo_apple.swiftui")
...
let test_swiftui_lazy_list_defers_missing_visible_row_render () =
  let source = read_file swiftui_source_path in
  require
    (contains source ~substring:"OCamlDemoLazyListRowView(")
  ...
  ;
  require
    (contains source ~substring:"if renderedChild === child")
    "lazy row cache hits should not rewrite the same rendered child @State during \
     repeated appear/disappear cycles";
  require
    (contains source ~substring:"renderedChildKey == nextRenderedChildKey")
    "lazy row cache-hit state writes should also compare the row key before publishing";
  require
    (contains source ~substring:"renderedChildRefreshGeneration == refreshGeneration")
    "lazy row cache-hit state writes should compare refresh generation so stale rows can \
     still update"
;;
...
let test_swiftui_lazy_list_uses_native_list_for_row_actions () =
  ...
  require
    (contains source ~substring:"private var shouldUseNativeList")
    "lazy provider lists should have one explicit gate for native List-only row \
     transactions";
  require
...
let test_swiftui_lazy_list_uses_scroll_view_for_read_only_provider () =
  let source = read_file swiftui_source_path in
  require
    (contains source
       ~substring:
         "if shouldUseNativeList {\n      nativeList(proxy)\n    } else {\n      lazyScrollList(proxy)\n    }")
    "read-only lazy provider lists should bypass native List because simulator profiling \
     showed UIKit List cell lifecycle/layout dominates scroll and sidebar hitches";
  require
    (contains source ~substring:"private func lazyScrollList(_ proxy: ScrollViewProxy) -> some View")
    "SwiftUI runtime should keep a lightweight ScrollView/LazyVStack path for read-only \
     lazy provider lists";
  require
    (contains source ~substring:"ScrollView {\n      LazyVStack")
    "the read-only lazy provider path should use LazyVStack inside ScrollView";
  require
    (contains source
       ~substring:
         "node.listDeleteEventId != nil || shouldEnableNativeListMove || node.listRefreshEventId != nil")
    "native List should still be used whenever native delete, native move, or refresh \
     transactions are active"
;;

let test_swiftui_native_list_installs_move_handlers_only_in_edit_mode () =
  let source = read_file swiftui_source_path in
  require
    (contains source ~substring:"private var shouldEnableNativeListMove")
    "native List move handlers should have one explicit gate";
  require
    (contains source ~substring:"node.isListEditMode && node.listMoveEventId != nil")
    "native List should only install move handlers while edit mode is active and a move \
     event exists; simulator sampling showed reorder accessory churn during normal \
     sidebar transitions";
  require
    (contains source ~substring:"if shouldEnableNativeListMove {\n          ForEach(lazyListPositions())")
    "lazy list move support should be absent from the view tree outside edit mode";
  require
    (contains source ~substring:"if shouldEnableNativeListMove {\n          nativeStaticListRows()")
    "static list move support should be absent from the view tree outside edit mode"
;;
...
let test_swiftui_lazy_list_move_updates_visible_order_immediately () =
  let source = read_file swiftui_source_path in
  require
    (not (contains source ~substring:"private struct OCamlDemoLazyListMovePreview"))
    ...
    (not
       (contains
          ...
          ~substring:
            ...
            \      OCamlDemoRenderedFrameScheduler.shared.runAfterRenderedFrame { [model] in"))
...
let test_swiftui_frame_probe_can_run_without_list_debug_bookkeeping () =
  let source = read_file swiftui_source_path in
  require
    (contains source ~substring:"OCAML_DEMO_FRAME_DEBUG")
  ...
  require
    ...
    "frame-only mode should be enabled independently from OCAML_DEMO_LIST_DEBUG";
  require
    (contains source
       ~substring:
...
let test_swiftui_frame_probe_logs_slow_lazy_row_renders () =
  ...
  require
    (contains source ~substring:"key=%@")
    "slow lazy row render logs should include the stable row key so simulator scroll \
     traces identify the expensive content";
  require
  ...
  require
    (contains source
       ~substring:"OCamlDemoFrameProbe.shared.markLazyRowRender")
...
let test_swiftui_host_model_is_only_observed_by_root_view () =
  let source = read_file swiftui_source_path in
  require
    (count_substrings source ~substring:"@ObservedObject var model: OCamlDemoHostModel" = 1)
    "only the root SwiftUI view should observe OCamlDemoHostModel; child rows use it as \
     an event dispatcher, otherwise every root update invalidates visible rows"
;;

let test_swiftui_node_modifier_stack_does_not_add_object_observers () =
  let source = read_file swiftui_source_path in
  let modifier_names =
    [
      "OCamlDemoKeyboardDismissControlsModifier";
      "OCamlDemoScrollDismissesKeyboardModifier";
      "OCamlDemoListRowSeparatorModifier";
      "OCamlDemoSearchModifier";
      "OCamlDemoNavigationTitleModifier";
      "OCamlDemoHorizontalSwipeModifier";
      "OCamlDemoNodeModifiers";
    ]
  in
  List.iter
    (fun name ->
       let start_marker = "private struct " ^ name in
       let start =
         match substring_index source ~substring:start_marker ~from:0 with
         | Some index -> index
         | None -> failwith ("SwiftUI modifier not found: " ^ name)
       in
       let next =
         match substring_index source ~substring:"\nprivate struct " ~from:(start + 1) with
         | Some index -> index
         | None -> String.length source
       in
       let section = String.sub source start (next - start) in
       require
         (not (contains section ~substring:"@ObservedObject var node"))
         (name
          ^ " should not add a second OCamlDemoNode ObservableObject subscription; \
             OCamlDemoNodeView already observes node and drives the modifier stack"))
    modifier_names
;;
...
let test_swiftui_lazy_list_row_is_equatable () =
  let source = read_file swiftui_source_path in
  require
    (contains source ~substring:"OCamlDemoLazyListRowView: View, Equatable")
...
let test_swiftui_lazy_list_visible_index_tracking_stays_off_scroll_hot_path () =
  ...
  require
    (not
       (contains source
          ~substring:
            "if OCamlDemoListVirtualizationProbe.shared.isDebugEnabled {\n\
...
let test_swiftui_lazy_list_keeps_slots_off_scroll_hot_path () =
  let source = read_file swiftui_source_path in
  require
    (not (contains source ~substring:"private struct OCamlDemoLazyListMoveSlotCollection"))
...
let test_swiftui_lazy_list_foreach_resolves_keys_only_for_visible_rows () =
  ...
  let stubs_source = read_file swiftui_stubs_source_path in
  require
    (contains source ~substring:"private struct OCamlDemoLazyListPositions")
  ...
  require
    (contains source
       ~substring:"id: ocaml_demoNativePublishedLazyRowKey(owner: owner, index: position)")
    "lazy row positions should read stable row keys from the published snapshot";
  require
    (not (contains source ~substring:"ocaml_demoNativeLazyRowIdentity"))
...
let test_swiftui_lazy_list_disappear_does_not_enqueue_release_work () =
  ...
  require
    (not (contains source ~substring:"OCamlDemoLazyRowDetachScheduler"))
...
let test_swiftui_lazy_list_refreshes_visible_rows_after_provider_update () =
  ...
  require
    (contains source ~substring:"let owner: OCamlDemoNode")
    "SwiftUI lazy row wrappers should not observe the whole list owner";
  require
    (not (contains source ~substring:"@ObservedObject var owner: OCamlDemoNode"))
    "list-level lazy list publishes should not invalidate every retained row wrapper";
  require
    (not (contains source ~substring:"@State private var child: OCamlDemoNode?"))
  ...
  require
    (contains source ~substring:"private final class OCamlDemoLazyListRowState")
    "non-visual lazy row lifecycle bookkeeping should live in a reference state holder";
  require
    (contains source ~substring:"private var cachedRenderedChild: OCamlDemoNode?")
    "SwiftUI lazy row wrappers should read rendered child nodes from the owner row cache";
  require
    (contains source ~substring:"private var displayedChild: OCamlDemoNode?")
...
let test_swiftui_lazy_list_content_cache_uses_stable_row_key () =
  let source = read_file swiftui_source_path in
  require
    (contains source ~substring:"var lazyListRowsByKey: [String: OCamlDemoNode] = [:]")
...
let test_swiftui_lazy_list_defers_append_row_count_while_scrolling () =
  ...
  require
    ...
    "append deferral should be based on the actual row-count increase, including any \
     pending coalesced row-count publish";
  require
  ...
  require
    (contains source
       ~substring:"OCamlDemoScrollStressProbe.shared.isActivelyScrolling(listID: node.id)")
...
let test_swiftui_lazy_list_row_count_decisions_use_pending_count () =
  let source = read_file swiftui_source_path in
  require
    (contains source ~substring:"private func effectiveLazyListRowCount")
    "lazy list row-count decisions should treat a pending coalesced publish as the \
     current count so rapid Enter/Delete does not repeatedly compare against stale \
     published row counts";
  require
    (contains source
       ~substring:
         "let currentRowCount = effectiveLazyListRowCount(node)\n  let appendDelta = rowCount - currentRowCount")
    "append/load-more deferral should compare new rows against the pending row count \
     when one exists";
  require
    (contains source
       ~substring:
         "oldCount: effectiveLazyListRowCount(node),\n      newCount: rowCount")
    "coalesced row-count logs should report the effective old count instead of the stale \
     published count";
  require
    (contains source
       ~substring:
         "effectiveLazyListRowCount(node) != rowCount,\n            !providerInvalidatedIndices.isEmpty")
    "structural row-count updates should still coalesce when the pending count differs \
     from the new count, even if the already-published count happens to match it";
  require
    (contains source
       ~substring:"if effectiveLazyListRowCount(node) == rowCount {")
    "same-count row-count updates should cancel pending publishes when the requested \
     count already matches the pending count"
;;

let test_swiftui_lazy_list_coalesced_publish_skips_unchanged_count () =
  let source = read_file swiftui_source_path in
  require
    (contains source
       ~substring:"guard node.lazyListRowCount != pendingRowCount else {")
    "coalesced structural row-count work should not publish when the pending count has \
     already reached the published count; simulator logs showed same-count publishes \
     during rapid Enter/Delete";
  require
    (contains source
       ~substring:"node.pendingLazyListRowCount = nil\n      node.pendingLazyListRowCountInvalidatedCount = 0")
    "same-count coalesced work should still clear the pending row-count state"
;;
...
let test_swiftui_lazy_list_publishes_deferred_appends_once_after_idle () =
  ...
  require
    (contains source ~substring:"OCamlDemoScrollIdleScheduler.shared.runWhenIdle")
...
let test_swiftui_lazy_list_deferred_append_waits_for_idle_without_max_latency () =
  let source = read_file swiftui_source_path in
  require
    (not (contains source ~substring:"maxDeferredLazyListRowCountPublishDelay"))
    "deferred load-more row-count publishes should not force row-count writes during \
     active scrolling because SwiftUI recalculates visible rows and drops frames";
  require
    (not (contains source ~substring:"pendingLazyListRowCountDeadline"))
    "lazy list nodes should not keep a max-latency deadline for append row-count publish";
  require
    (not (contains source ~substring:"let exceededMaxDelay"))
    "deferred row-count publish should not compute a deadline override";
  require
    (not (contains source ~substring:"&& !exceededMaxDelay"))
    "the idle deferral check should keep waiting while scrolling stays active";
  require
    (contains source ~substring:"OCamlDemoScrollIdleScheduler.shared.runWhenIdle")
    "deferred row-count publish should still resume as soon as the scroll view is idle"
;;
...
let test_swiftui_lazy_list_emits_row_count_published_event () =
  ...
  require
    (contains source ~substring:"weak var hostModel: OCamlDemoHostModel?")
...
let test_swiftui_exposes_scroll_idle_main_dispatch () =
  let source = read_file swiftui_source_path in
  let stubs = read_file "../swiftui/ocaml_demo_apple_swiftui_stubs.c" in
  let ml_source = read_file swiftui_backend_source_path in
  let mli_source = read_file "../swiftui/ocaml_demo_apple_swiftui.mli" in
  require
    (contains source ~substring:"private final class OCamlDemoScrollIdleScheduler")
  ...
  require
    (contains source ~substring:"OCamlDemoScrollStressProbe.shared.isAnyRunning()")
    "scroll stress should exercise the same idle-dispatch path as manual scrolling";
  require
    (contains source ~substring:"ocaml_demo_native_swiftui_run_on_main_when_scroll_idle")
    "Swift should expose a C entrypoint for OCaml callbacks that must wait until scroll \
     idle";
  require
    (contains stubs ~substring:"ocaml_demo_apple_swiftui_run_on_main_when_scroll_idle")
...
let test_swiftui_exposes_rendered_frame_main_dispatch () =
  ...
  require
    (contains source
       ~substring:"ocaml_demo_native_swiftui_run_on_main_after_rendered_frame")
    "Swift should expose a C entrypoint for OCaml callbacks that must wait until the \
     current UI update has presented";
  require
    (contains source
       ~substring:"OCamlDemoRenderedFrameScheduler.shared.runAfterRenderedFrame")
    "rendered-frame main dispatch should share the CADisplayLink scheduler used by native \
     list move preview commits";
  require
    (contains stubs ~substring:"ocaml_demo_apple_swiftui_run_on_main_after_rendered_frame")
...
let test_swiftui_mac_bridge_exports_lazy_list_symbols () =
  ...
  [
    ...
    "ocaml_demo_native_swiftui_clear_lazy_list_rows";
...
let test_swiftui_scroll_stress_pauses_at_end_for_deferred_appends () =
  let source = read_file swiftui_source_path in
  require
    (contains source ~substring:"func isActivelyScrolling(listID: UUID) -> Bool")
    "scroll stress should expose only active scrolling to the lazy list row-count deferral \
     guard";
  require
    (contains source ~substring:"private func pauseAtEnd(listID: UUID)")
    "scroll stress should pause, not discard, the run when it reaches the currently \
     loaded end";
  require
  ...
  require
    (contains source ~substring:"resumeFromCurrentOffset")
    "scroll stress should resume from the current scroll offset after appended rows publish \
     instead of restarting at row zero";
  require
...
let test_swiftui_lazy_list_logs_ui_row_lifecycle_counts () =
  let source = read_file swiftui_source_path in
  require
    (contains source ~substring:"OCamlDemoRowLifecycleProbeView(listID: listID)")
    "lazy row lifecycle logging should use a platform view probe attached to each row";
  require
    (contains source ~substring:"ocaml_demoNativeLifecycleProbeBackground")
    "lifecycle probes should only be attached when list debug logging is enabled";
  require
    (not (contains source ~substring:"OCamlDemoLazyRowLifecycleToken"))
    "lazy row lifecycle logging should not use StateObject tokens because they change row \
     retention behavior";
  require
    (not (contains source ~substring:"OCamlDemoMediaViewLifecycleToken"))
...
let test_swiftui_lazy_list_uses_stable_row_keys_for_identity () =
  let source = read_file swiftui_source_path in
  require
    (contains source ~substring:"ocaml_demoNativePublishedLazyRowKey(owner: owner, index: position)")
  ...
  require
    (contains source ~substring:"let resolvedKey = ocaml_demoNativeLazyRowKey(")
...
let test_swiftui_lazy_list_setters_skip_unchanged_published_values () =
  ...
  require
    (contains
       source
       ~substring:"if effectiveLazyListRowCount(node) == rowCount {")
    "lazy list updates should not republish the same row count, including when the same \
     count is already pending publication";
  require
    (contains source ~substring:"node.pendingLazyListRowCount = nil")
    "same-count lazy list updates should clear deferred row-count state";
  require
    (contains source ~substring:"node.pendingLazyListRowCountWorkItem = nil")
    "same-count lazy list updates should clear deferred publish work";
  require
  ...
  ;
  require
    (contains source ~substring:"if node.text != nextText")
    "text updates should not republish unchanged text during repeated row renders";
  require
    (contains source ~substring:"if node.clickEventId != nextEventId")
    "click callback updates should not republish unchanged event ids during repeated row \
     renders";
  require
...
let test_swiftui_structural_text_edits_handoff_keyboard_until_next_focus () =
  let source = read_file swiftui_source_path in
  require
    (contains source ~substring:"private final class OCamlDemoKeyboardHandoff")
  ...
  require
    (contains source ~substring:"OCamlDemoKeyboardHandoff.shared.retainKeyboard(from: self)")
    "delete-at-start should retain the keyboard before dispatching the structural OCaml \
     event";
  require
    (contains source ~substring:"OCamlDemoKeyboardHandoff.shared.retainKeyboard(from: textField)")
    "Return should retain the keyboard before dispatching the insert event";
  require
    (contains source ~substring:"OCamlDemoKeyboardHandoff.shared.completeHandoff()")
    "new focused text fields should release the handoff after becoming first responder";
  require
    (contains source ~substring:"\"keyboard_handoff_retain\"")
    "keyboard handoff should log retain events so Enter/delete stalls are debuggable";
  require
    (contains source ~substring:"\"keyboard_handoff_complete\"")
    "keyboard handoff should log completion when the next editor becomes first responder";
  require
    (contains source ~substring:"\"keyboard_structural_submit\"")
    "Return and delete-at-start should log structural edit submissions";
  require
    (contains source
       ~substring:
         "ProcessInfo.processInfo.environment[\"OCAML_DEMO_INTERACTION_DEBUG\"] == \"1\"")
    "interaction debug logging should be env-gated so normal structural edits do not \
     write stderr/os_log on the hot path";
  require
...
let test_swiftui_text_field_blur_dispatches_event () =
  let source = read_file swiftui_source_path in
  let ml_source = read_file "../swiftui/ocaml_demo_apple_swiftui.ml" in
  require
    (contains source ~substring:"textFieldBlurEventId")
    "native text fields should store an explicit blur event id";
  require
    (contains source ~substring:"onBlur")
    "UIKit text field wrappers should call an onBlur callback when editing ends";
  require
    (contains source ~substring:"textFieldDidEndEditing")
    "single-line text fields should dispatch blur from UITextFieldDelegate";
  require
    (contains source ~substring:"textViewDidEndEditing")
    "multi-line text fields should dispatch blur from UITextViewDelegate";
  require
    (contains ml_source ~substring:"set_text_field_blur")
    "OCaml SwiftUI backend should expose a text field blur event to apps"
;;
...
let test_swiftui_vertical_delete_aware_text_fields_wrap () =
  let source = read_file swiftui_source_path in
  require
    (contains source ~substring:"private final class OCamlDemoDeleteAwareUITextView")
    "vertical delete-aware text fields should use UITextView because UITextField cannot \
     wrap text";
  require
    (contains source ~substring:"OCamlDemoDeleteAwareTextView(")
  ...
  require
    (contains source ~substring:"OCamlDemoKeyboardHandoff.shared.retainKeyboard(from: textView)")
...
let test_compact_sidebar_keyboard_tracking_stays_on_drawer_root () =
  ...
  let split_start =
    match
      substring_index source ~substring:"private struct OCamlDemoCompactSidebarSplitView"
        ~from:0
    with
    | Some index -> index
    | None -> failwith "OCamlDemoCompactSidebarSplitView not found"
  in
  let content_start =
    match substring_index source ~substring:"private struct OCamlDemoNodeView" ~from:split_start with
    | Some index -> index
    | None -> failwith "OCamlDemoNodeView not found"
  in
  ...
  let sidebar_content_start =
    match
      substring_index split_source ~substring:"private var compactSidebarContent" ~from:0
    with
    | Some index -> index
    | None -> failwith "compactSidebarContent definition not found"
  in
  let sidebar_content_end =
    match substring_index split_source ~substring:"  private var sidebarHeaderActionButton" ~from:sidebar_content_start with
    | Some index -> index
    | None -> failwith "compactSidebarContent definition end not found"
  in
  let sidebar_content_source =
    String.sub split_source sidebar_content_start (sidebar_content_end - sidebar_content_start)
  in
  require
    (contains split_source ~substring:"UIResponder.keyboardWillChangeFrameNotification")
    "compact sidebar keyboard notifications should stay on the split-view root";
  require
    (not
       (contains
...
let test_sheet_content_host_fills_sheet_background () =
  let source = read_file swiftui_source_path in
  require
    (contains source ~substring:"ocaml_demoSheetContentHost")
  ...
  require
    (contains source ~substring:".background(ocaml_demoHomeBodyBackground")
...
let test_sheet_content_host_preserves_leading_content_alignment () =
  let source = read_file swiftui_source_path in
  let host_start =
    match substring_index source ~substring:"private func ocaml_demoSheetContentHost" ~from:0 with
    | Some index -> index
    | None -> failwith "ocaml_demoSheetContentHost not found"
  in
  let detents_start =
    match substring_index source ~substring:"private var sheetPresentationDetents" ~from:host_start with
    | Some index -> index
    | None -> failwith "sheetPresentationDetents not found after ocaml_demoSheetContentHost"
  in
...
let test_text_field_blur_event () =
  Backend.reset ();
  let component graph =
    let blurred, set_blurred = Apple.state graph ~key:"blurred" false in
    Apple.vstack
      [
        Apple.text_field ~text:"Editing"
          ~placeholder:"Block"
          ~is_focused:true
          ~on_change:(fun _ -> Apple.Action.ignore)
          ~on_blur:(set_blurred true)
          ();
        Apple.text (if blurred then "blurred" else "editing");
      ]
  in
  let app = App.create component in
  App.flush_and_render app;
  let root =
    ...
    | None -> failwith "app did not render"
  in
  Backend.blur_text_exn root ~path:[ 0 ];
  App.flush_and_render app;
  let rendered =
    match App.view app with
    | Some root -> Backend.show root
  ...
  in
  require
    (contains rendered ~substring:"text=\"blurred\"")
    "text field blur should dispatch its event"
;;
...
let test_plain_text_fields_match_body_text_size () =
  let source = read_file swiftui_source_path in
  require
    (contains source ~substring:".font(ocaml_demoNativePreferredFont(.body))")
    "plain SwiftUI text fields should use the same body font as display text";
  require
    (contains source ~substring:"ocaml_demoNativePreferredUIFont(.body)")
    "delete-aware UIKit text inputs should use the same body font as display text";
  require
    (not (contains source ~substring:"textView.font = ocaml_demoNativePreferredUIFont(size: 18"))
    "multiline block editors should not use a larger hard-coded font than display rows";
  require
    (not (contains source ~substring:"textField.font = ocaml_demoNativePreferredUIFont(size: 18"))
...
let test_swiftui_frame_supports_leading_alignment () =
  ...
  require
    ...
    "SwiftUI frame modifiers should use the published alignment instead of default \
     center alignment";
  require
    (contains source ~substring:"node.frameAlignment = OCamlDemoFrameAlignment(rawValue:")
...
let test_swiftui_horizontal_stack_supports_top_alignment () =
  ...
  require
    (contains source ~substring:"ocaml_demo_native_swiftui_set_horizontal_stack_alignment")
    "native stack alignment setter should publish the OCaml alignment value"
  ;
  require
    (contains source ~substring:"OCamlDemoHorizontalStackAlignment(rawValue: alignment)")
...
let test_swiftui_custom_view_supports_youtube_webkit_iframes () =
  ...
  require
    (contains source ~substring:"OCamlDemoYouTubeIframeView")
...
let test_custom_view_supports_change_messages () =
  ...
  require
    ...
    "custom views should optionally dispatch string messages into OCamlDemo";
  require
...
let test_swiftui_custom_view_supports_interactive_bundle_webviews () =
  let source = read_file swiftui_source_path in
  require
    (contains source ~substring:"OCamlDemoAppWebView")
  ...
  require
    ...
    "JavaScript messages should be dispatched through the OCamlDemo event callback";
  require
...
let test_youtube_iframe_does_not_steal_list_row_gestures () =
  let source = read_file swiftui_source_path in
  require
    (contains source ~substring:"OCamlDemoDeferredYouTubeIframeView(payload: payload)")
  ...
  require
    (contains source ~substring:"OCamlDemoYouTubeIframeView(payload: payload)")
...
let test_swiftui_prefers_inter_for_typography () =
  let source = read_file swiftui_source_path in
  require
    (contains source ~substring:"private let ocaml_demoNativePreferredFontFamily = \"Inter\"")
    "SwiftUI runtime should define Inter as the preferred app font family";
  require
    (contains source ~substring:"Font.custom(ocaml_demoNativePreferredFontFamily")
  ...
  require
    (contains source ~substring:".font(ocaml_demoNativePreferredFont(size:")
...
let test_keyboard_accessory_dismiss_sends_blur () =
  let source = read_file swiftui_source_path in
  require
    (contains source ~substring:"dismissKeyboardForUser()")
    "keyboard accessory dismiss should resign user focus without suppressing the blur event";
  require
    (not
       (contains source
          ~substring:"onDismiss: {\n          textView.clearFocusRequest()"))
    "text view keyboard accessory dismiss must not use the model-driven focus clear path";
  require
    (not
       (contains source
          ~substring:"onDismiss: {\n          textField.clearFocusRequest()"))
    "text field keyboard accessory dismiss must not use the model-driven focus clear path"
;;
...
let test_navigation_path_stack_uses_grouped_background_chrome () =
  let source = read_file swiftui_source_path in
  require
    (contains source ~substring:".toolbarBackground(ocaml_demoHomeBodyBackground, for: .navigationBar)")
    "Navigation path stacks should tint the navigation bar with the same grouped background";
  require
    (contains source
       ~substring:".toolbarBackground(ocaml_demoHomeBodyBackground, for: .bottomBar)")
    "Navigation path stacks should tint the bottom toolbar with the same grouped background";
  require
    (contains source ~substring:".listRowBackground(ocaml_demoHomeBodyBackground)")
...
let () =
  ...
  test_compact_sidebar_state_lives_in_dedicated_view ();
  test_compact_sidebar_avoids_type_erased_selected_route_boundary ();
  test_compact_sidebar_composites_main_page_before_clipping ();
  test_compact_sidebar_has_env_gated_body_probe ();
  test_compact_sidebar_keeps_drawer_drag_arbitration_local ();
  test_compact_sidebar_uses_drawer_for_content_swipes ();
  test_compact_sidebar_does_not_install_closed_root_drag ();
  test_compact_sidebar_uses_fast_logged_animation ();
  test_compact_sidebar_has_native_stress_probe ();
  test_horizontal_swipe_ignores_disabled_directions ();
  test_horizontal_swipe_ignores_vertical_scroll_before_state_update ();
  ...
  test_swiftui_lazy_list_uses_scroll_view_for_read_only_provider ();
  test_swiftui_native_list_installs_move_handlers_only_in_edit_mode ();
  ...
  test_swiftui_lazy_list_row_count_decisions_use_pending_count ();
  test_swiftui_lazy_list_coalesced_publish_skips_unchanged_count ();
  ...
  test_swiftui_lazy_list_deferred_append_waits_for_idle_without_max_latency ();
  ...
  test_swiftui_node_modifier_stack_does_not_add_object_observers ();
  ...
  test_text_field_blur_event ();
  test_text_field_focus_renders ();
  test_swiftui_text_field_blur_dispatches_event ();
  ...
  test_keyboard_accessory_dismiss_sends_blur ();
