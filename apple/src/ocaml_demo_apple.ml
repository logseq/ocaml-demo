...
let debug_enabled () =
  match Sys.getenv_opt "OCAML_DEMO_LIST_DEBUG" with
...
let debug_log format =
  Printf.ksprintf
    (fun message ->
      if debug_enabled ()
      then (
        prerr_endline ("[OCamlDemoleDebug] " ^ message);
        flush stderr))
    format
;;
...
type graph = Ocaml_demo_native.graph

let state = Ocaml_demo_native.state
let scope = Ocaml_demo_native.scope
...
let image_payload_header = "ocaml_demo-image-payload"
...
module Renderer = struct
  module type Backend = sig
    ...
    val set_text_field_blur : view -> (unit -> unit) option -> unit
  ...
  module Make (Backend : Backend) = struct
    ...
    and fingerprint_parts node modifiers =
      ...
      let shape =
        ...
        | Custom_view_node { key; kind; _ } ->
          "custom-view:" ^ opt key ^ ":" ^ kind
        | Modified_node _ -> assert false
      in
    ...
    and patch_same_kind t node modifiers =
      ...
      (match node with
       ...
       | Text_field_node
         ...
         Backend.set_text_field_blur
           t.view
           (Option.map on_blur ~f:(fun on_blur -> fun () -> t.schedule_event on_blur));
       ...
       | Custom_view_node { kind; on_change; _ } ->
         ...
         Backend.set_on_change
           t.view
           (Option.map on_change ~f:(fun on_change -> fun message ->
...
module App = struct
  module Make (Backend : Renderer.Backend) = struct
    ...
    type t = (node, R.t) Ocaml_demo_native.App_driver.t

    let create component =
      Ocaml_demo_native.App_driver.create
        component
    ...
    let flush_and_render = Ocaml_demo_native.App_driver.flush_and_render
    let view t = Option.map (Ocaml_demo_native.App_driver.rendered t) ~f:R.view
  end
end
...
module For_testing = struct
  module Backend = struct
    ...
    type view =
      ...
      ; mutable on_text_blur : (unit -> unit) option
    ...
    let kind_name view =
      ...
      | Custom_view _ ->
        "custom(" ^ Option.value view.text ~default:"" ^ ")"
    ;;
    ...
    let blur_text_exn view ~path =
      let view = find_visible_exn view ~path in
      match view.on_text_blur with
      | Some f -> f ()
      | None -> failwith "View has no text blur handler"
    ;;
